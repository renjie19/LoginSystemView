...
public class ReportController implements Initializable,Refreshable<Report> {
    @FXML
    ...
    void onItemClick(MouseEvent event) {
        if(reportTable.getSelectionModel().getSelectedIndex()>=0) {
            ...
            try{
                ...
                Editable<Refreshable,Report> timeLogController = loader.getController();
    ...
    @Override
    public void refresh(ActionEvent event, Report report) {
        onSearchClick(event);
